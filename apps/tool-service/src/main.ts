...
import { ToolServiceModule } from './tool-service.module';
...
async function bootstrap() {
  const app = await NestFactory.create(ToolServiceModule);
  ...
  const port =
    Number(config.get('TOOL_SERVICE_PORT')) || Number(process.env.PORT) || 3002;
  ...
  const queue = config.get('TOOL_SERVICE_QUEUE') || 'tool_service_queue';
  ...
  console.log(`✅ Tool Service is running on port ${port}`);
}
void bootstrap();