...
import { ToolMarketingServiceModule } from './tool-marketing-service.module';
...
async function bootstrap() {
  const app = await NestFactory.create(ToolMarketingServiceModule);
  // Enable CORS
  app.enableCors({
  ...
  const port =
    Number(config.get('TOOL_MARKETING_SERVICE_PORT')) || Number(process.env.PORT) || 3010;
  ...
  const queue = config.get('TOOL_MARKETING_SERVICE_QUEUE') || 'tool_marketing_service_queue';
  ...
  console.log(`✅ Tool Marketing Service is running on port ${port}`);
}
void bootstrap();
