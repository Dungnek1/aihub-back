...
async function bootstrap() {
  ...
  const port =
    Number(config.get('COURSE_SERVICE_PORT')) || Number(process.env.PORT) || 3002;

  // Connect RMQ microservice listener so this service can receive RPC calls
  const rmqUrlLocal = config.get('RABBITMQ_URL_LOCAL') as string | undefined;
  ...
  const queue = config.get('COURSE_SERVICE_QUEUE') || 'course_service_queue';

  const urls = rmqUrl ? [rmqUrl] : [`amqp://${rmqUser}:${rmqPass}@${rmqHost}:${rmqPort}`];

  console.log('✅ Connecting to RMQ with URLs:', urls);
